...
3. Run prerequisite checks first with `node skills/create-rill-package/scripts/preflight.mjs`; halt on failure and report output.
4. Fetch rill docs/indices with `curl -sL` (not summarized fetches). Reuse fetched docs across phases.
...
6. Bootstrap/install/probe in the real package directory (Phase 4.5):
   ...
   If install/probe fails, surface stderr and follow the same blocking branch behavior as the skill; do not silently continue.
7. Preserve file ownership boundaries:
...
9. Apply skill decision gates exactly:
